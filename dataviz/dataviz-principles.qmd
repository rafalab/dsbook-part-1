...
This plot makes a 13% increase look like a five fold change. Below, it is plotted again as it should be; and then skewed as above with a misleading y axis:

```{r barplot-from-zero-2, echo=FALSE}
tax <- data.frame(date = c("Now", "Jan 1, 2013"), tax_rate = c(35, 39.6)) |>
...
tax2 <- tax + coord_cartesian(ylim=c(34,42))
grid.arrange(tax, tax2, ncol=2)
```
...
Redefine the `state` object so that its levels are re-ordered by rate. Print the new object `state` and its levels so you can see that the vector is now re-ordered by the levels.


5\. Now with one line of code, define the `dat` table as done above, but use mutate to create a rate variable and re-order the state variable by rate. Then make a barplot using the code above, but for this new `dat`.
