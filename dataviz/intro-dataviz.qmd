...
Another striking example comes from a New York Times chart[^intro-dataviz-2], which summarizes scores from the NYC Regents Exams. As described in the article[^intro-dataviz-3], these scores are collected for several reasons, including to determine if a student graduates from high school. In New York City you need a 65 to pass. The distribution of the test scores forces us to notice something somewhat problematic:
