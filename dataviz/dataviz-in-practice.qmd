...
To have the areas of these densities be proportional to the size of the groups, we could possibly multiply the y-axis values (density) by the size of the group; but from the `geom_density` help file, we see that the functions already compute a `count` variable that stores exactly this - `density * points`. We want to position this computed variable, rather than density, on the y-axis.
