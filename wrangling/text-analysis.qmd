...
:::{.callout-note}
`relationship = "many-to-many"` is added to address a warning that arises from `inner_join` detecting an "unexpected many-to-many relationship". However, this behavior is actually expected in this context because many words have multiple sentiments associated with them.  This can occur with outer joins as well.
:::
...
And we can make a graph - of the sentiments with the largest tweet count discrepancies between the two devices; with the most frequently tweeted words of each sentiment, each word colored to reflect which device was prominent: 
