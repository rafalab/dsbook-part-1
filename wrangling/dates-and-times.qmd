...
The package also includes the classes `IDate` and `ITime`, which store dates and times more efficiently than `lubridate` and base R. This is convenient for large files with date stamps. You can convert dates in the usual R format: using `as.IDate` and `as.ITime`. You can see this by using the `object.size` function:
