...
We can then join these to generate one name for each column. This is a common task, when source data headings span multiple lines. In this case, we want "discipline" plus the permutations of the other (multi-line) columns - awards_[total|men|women], applications_[total|men|women], etc.  We use `rep` and `paste` for the permutations with an underscore separator, and use the index for "discipline" to exclude/include it as appropriate.
