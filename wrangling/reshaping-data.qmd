...
We will use the fertility wide format dataset described in Section @sec-tidy-data as an example in this section.
...
Three related function are `separate_wider_position`, `separate_wider_regex`, and `unite`. `separate_wider_position` takes a width instead of delimiter. `separate_wider_regex`, described in @sec-separate_regex, provides much more control over how we separate and what we keep. The `untie` function can be tought of as the inverse of the `separate` function: it combines two columns into one. 
