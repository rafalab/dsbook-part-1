...
The __dslabs__ package includes a file that is not UTF-8 encoded to serve as an example. Notice the strange characters that appear you attempt to read inthe first line:
...
Importing fuctions, or parsers, are avaible from base R. However, more powerful and often faster functions are available in the __readr__, __readxl__, and __data.table__ pacakges. In this section we review some examples. We also describe how data can be downlaoded or read directly from the internet.
...
The __readr__ parse permit us to specify an encoding. It also includes a function that tries to guess the encoding:
