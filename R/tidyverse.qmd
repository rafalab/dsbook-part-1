...
:::{.callout-tip, title = "Advanced"}
If the function begin called inside `summarize` returns a data frame, it must have only one row. If the function returns more than one row, you can use the function `reframe`.
:::
...
In Section @sec-vectorization we learned about the `sapply` function, which permitted us to apply the same function to each element of a vector. We constructed a function and used `sapply` to compute the sum of the first `n` integers for several values of `n` like this:
